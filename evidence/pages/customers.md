...
`lifetime_order_count` and `lifetime_net_revenue_eur` are columns on the `dim_customers` mart,
...
where has_ordered
order by lifetime_net_revenue_eur desc
limit 20
```
...
<DataTable data={top_customers} rows=20 search=true>
    ...
    <Column id=lifetime_net_revenue_eur title="Lifetime net revenue" fmt=eur0 contentType=bar barColor="#A8C9EE" />
</DataTable>
...
select
    customer_segment,
    count(*)                          as customers,
    sum(lifetime_net_revenue_eur)     as net_revenue_eur
from refarch.customers
where has_ordered
group by 1
